...
TRIPLETS=$(curl -s "${RUST_DIST_URL}" | grep "${RUST_STD_PREFIX}" | grep href | grep unknown-linux | grep gnu | grep tar.gz.sha256 | sed -e "s#.*href=\\\"/dist/${RUST_STD_PREFIX}\(.*\).tar.gz.sha256\\\".*#\1#g")
